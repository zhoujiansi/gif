...
function draw(start, width) {
    ...
    if (canvas.getContext) {
        ...
        if (window.devicePixelRatio) { //设备上物理像素   该方案处理画布锯齿问题
            imgStatus.scale=window.devicePixelRatio;
        ...
        picImg = new Image();
        picImg.src = imgBase64;
        var naturalWidth = picImg.width;
        var naturalHeight = picImg.height;
        sourceInterval=setInterval(function(){
            let naturalWidth = picImg.width;
            let naturalHeight = picImg.height;
            if(naturalWidth>0&&naturalHeight>0){
                imgStatus.width=naturalWidth;
                imgStatus.height=naturalHeight;
                ...
                // alert(imgBase64.length+"#"+naturalWidth+"#"+naturalHeight);
                //这里的onload 在IOS里面不会执行，具体原因还不清楚
                ctx.drawImage(picImg,0,0,imgStatus.showWidth,imgStatus.showHeight);
                $(".page2").hide();//
                $(".pagecj").show();//
                clearInterval(sourceInterval);
                // alert("请用手指缩放移动头像到合适的位置");
            }
        },100);
        // picImg.src = $("#source").attr("src");
    }
}
//选择完毕 重要
$(".icon_btn1").click(function(){
    ...
    //在640*1008的设计大小下
    let beishu=window.devicePixelRatio||1;
    let _width=$(".page").width();
    let _wid=_width*400/640;
    let _heg=_wid*522/400;
    let bili=_width/640;//缩放比例
    let _chaX=bili*117*beishu;//需要截图的(左上角)起点距离左上角的X轴距离
    let _chaY=bili*202*beishu;//需要截图的(左上角)起点距离左上角的Y轴距离
    ...
    if (_tempcanvas.getContext) {
        ...
        let wd = _tempcanvas.width,
        hg = _tempcanvas.height;
        //放大画布，然后通过css 缩放，提高画布精度，防止出现锯齿 需要裁剪的话不能使用
        if (window.devicePixelRatio) { //设备上物理像素   该方案处理画布锯齿问题
            // imgStatus.scale=window.devicePixelRatio;
            _tempcanvas.style.width = wd + "px";
            _tempcanvas.style.height = hg + "px";
            _tempcanvas.height = hg * window.devicePixelRatio;
            _tempcanvas.width = wd * window.devicePixelRatio;
            ctx.scale(window.devicePixelRatio, window.devicePixelRatio);
        }
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        ctx.drawImage(canvas,_chaX,_chaY, _wid * beishu, _heg * beishu,0,0, _wid, _heg);
...
function lastProgram(){
    ...
    var _wid3=$(".page").width();
    var _height=_wid3*(466/640)*(680/800)+4;
    var _top=_wid3/640*250;
    console.log(_height,_top);
    // $(".page3 .content").css("height",_height);
    $(".page3 .pgcj_hand").css("top",_height+_top+"px");
    $(".pagecj").hide();
    $(".page3").show();
}
//画布转Gif图片
...
$("#cjbg").on('touchstart touchmove touchend',function(event) {
